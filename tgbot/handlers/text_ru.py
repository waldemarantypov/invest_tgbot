...
start_created_portfolio = 'Перейти к портфелю: /portfolio\n\n'
start_created_feedback = 'Если будет минута чтобы ответить на 5 вопросов: /feedback\n' \
                         'Это поможет мне сделать бот полезней.\n\n'
start_created_portfolio_example = "Вот так будет выглядеть портфель:"
start_created_share = 'Поделиться ботом можно просто переслав это сообщение с твоей уникальной ссылкой: {share_url}'
start_created_share_command = 'В любое время ты можешь получить свою уникальную ссылку для того, чтобы' \
                              ' поделиться ботом с друзьями здесь: /share'
start_change_language = 'Изменить язык: /language'
...
# command share
share_command = "Чтобы поделиться ботом - перешли товарищам сообщение ниже:\n\n"
share_message = "Привет!\n\n" \
                "Меня зовут Inbaze и я портфолио-бот 🤖\n" \
                "Я помогаю отслеживать рост 📈 / падение 📉 твоего инвестиционного портфеля (акции и криптовалюта).\n\n" \
                "Бот полностью бесплатный, поэтому welcome!\n" \
                "{share_url}"
...
close_modify_menu_portfolio = 'Хорошо.\n\n' \
                              ...
                              'Если хочешь поделиться ботом с друзьями: /share'
# OKAY
...
add_stock_text = 'Введи cимвол акции / криптомонеты, которую хочешь ДОБАВИТЬ ➕\n\n' \
                 '<i>(У Tesla символ - TSLA, а у биткоина к доллару - "BTC-USD")</i>.\n\n' \
                 'p.s. Если ты не уверен в правильности написания символа' \
                 ' - проверь его здесь: https://finance.yahoo.com'
...
modify_balance_stock_symbol_shares = 'В портфеле сейчас {shares} {symbol}.\n' \
                                     'На сколько изменяем?\n\n' \
...
delete_stock_success = 'Ты удалил <b><i>{symbol}</i></b> из портфеля, надеюсь выгодно ушла 🙏'
...
add_stock_not_in_yfinance = 'Ошибка: ты не правильно ввел символ акции / криптомонеты.\n\n' \
                            '<i>Посмотри правильное написание символа тут: https://finance.yahoo.com</i>'

language_ask = 'Выберите язык:'
language_EN = "🇺🇸"
language_RU = "🇷🇺"
language_modefied_succes = "Язык успешно изменен!"
# okay
# дальше не трогаем
